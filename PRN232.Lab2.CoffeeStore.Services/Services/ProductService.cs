...
namespace PRN232.Lab2.CoffeeStore.Services.Services
{
    public class ProductService : IProductService
    {
        ...
        public ProductService(ILogger<ProductService> logger, IUnitOfWork unitOfWork, IDistributedCache distributedCache)
        {
            _logger = logger;
        ...
        public async Task<DataServiceResponse<PaginationServiceResponse<ProductResponse>>> GetProducts(GetProductsRequest request)
        {
            ...
            var query = productRepository.Query()
                ...
                .Include(p => p.Category)
                    .Where(p => p.Category != null && p.Category.IsActive)   // lọc theo Category
            ...
            {
                TotalPage = request.Page * request.PageSize,
                TotalCount = totalProducts,
                TotalCurrent = productResponses.Count,
                Page = request.Page,
        ...
        {
            var cacheKey = $"products:{request.Page}:{request.PageSize}:{request.Sort}:{request.Search}:{request.Select}";

            var cachedData = await _distributedCache.GetStringAsync(cacheKey);

            if(!string.IsNullOrEmpty(cachedData))
            {
                return JsonSerializer.Deserialize<DataServiceResponse<PaginationServiceResponse<object?>>>(cachedData)!;
